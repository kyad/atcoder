...
int main() {
  int W, N;
  cin >> W >> N;
  vector<int> L(N + 1, 0), R(N + 1, 0), V(N + 1, 0);
  for (int n = 1; n <= N; n++) {
    cin >> L.at(n) >> R.at(n) >> V.at(n);
  }
  const long long INF = 4e18;
  // 1点更新・区間最大値
  auto fx = [](long long x1, long long x2) -> long long { return max(x1, x2); };
  vector<SegTree<long long> > dp(N + 1, SegTree<long long>(W + 1, fx, -INF));
  // 初期化
  dp[0].set(0, 0);
  for (int n = 1; n <= N; n++) {
    dp[0].set(n, -INF);
  ...
  for (int i = 1; i <= N; i++) {
    for (int j = 0; j <= W; j++) {
      long long now = dp[i - 1].get(j);
      if (j - L[i] >= 0) {
        int l = max(0, j - R[i]);
        int r = max(0, j - L[i]);
        now = max(now, dp[i - 1].query(l, r + 1) + V[i]);
      }
      if (now < 0) {
        now = -INF;
      }
