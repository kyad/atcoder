...
int main() {
  ...
  seg.build();
  vector<int> height_to_i(N + 1, 0);
  for (int i = 0; i < N; i++) {
    int height = P.at(i);
    height_to_i.at(height) = i;
  }

  // [l, r]で最高の高さのidxを返す
  auto h = [&](int l, int r) {
    int height = seg.query(l, r + 1);
    int idx = height_to_i.at(height);
    return idx;
  };
  auto f = [&](auto f, int l, int r) -> long long {
    ...
    // left
    if (k != l) {
      long long dist = k - h(l, k - 1);
      long long now = f(f, l, k - 1) + dist;
      ans = max(ans, now);
    }
    if (k != r) {
      long long dist = h(k + 1, r) - k;
      long long now = f(f, k + 1, r) + dist;
      ans = max(ans, now);
    }
    return ans;
  };
  long long ans = f(f, 0, N - 1);
  cout << ans << '\n';
  return 0;
}
