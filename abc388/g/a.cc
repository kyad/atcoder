// https://www.youtube.com/live/UWoRBhN2s6Y?t=7635s
...
int main() {
  ...
  // Y[i]: A[i] <= 2*A[j] となるような最小のjに対してj-i
  vector<int> Y(N);
  int j = 0;
  for (int i = 0; i < N; i++) {
    while (j < N && 2 * A.at(i) > A.at(j)) {
      j++;
    }
    Y.at(i) = j - i;
  }
  ...
  for (int q = 0; q < Q; q++) {
    ...
    auto judge = [&](int K) -> bool {
      return seg.query(L, L + K) <= R - K - L;
    };
    int left = 0;  // OK
    int right = (R - L) / 2 + 1;  // NG
    while (right - left > 1) {
