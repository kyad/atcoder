...
// Argument: A: 0-indexed, a permutation of {0, 1, 2, ..., N - 1}
// Use long long bit, because N=10^5 requires long long bit
// Return value: Inversion number of permutation A
template<class T> long long inversion_number_perm(vector<T> A) {
  int N = A.size();
...
// Compress A. Duplicated values are assigned to different values.
// Argument: A (input, output) 0-indexed array.
//   A is overwritten by a permutation of {0, 1, 2, ..., A.size() - 1}
// O(N)
template<class T> void compress_perm(vector<T> &A) {
  map<T, vector<int> > mp;  // Value -> indices
  for (size_t i = 0; i < A.size(); i++) {
    mp[A[i]].push_back(i);
  }
  int count = 0;
  for (class map<T, vector<int> >::iterator it = mp.begin(); it != mp.end(); it++) {
...
template<class T> long long inversion_number(vector<T> A) {
  vector<T> B(A);
  compress_perm(B);
  return inversion_number_perm(B);
}
...
int main() {
  ...
  long long ans = inversion_number_perm(Y);
