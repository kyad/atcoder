...
int main() {
  int H, W, N;
  cin >> H >> W >> N;
  vector<tuple<int, int, int> > RC(N);
  for (int i = 0; i < N; i++) {
    int r, c;
    cin >> r >> c;
    RC.at(i) = make_tuple(r, c, i);
  }
  sort(RC.begin(), RC.end());
  vector<int> R(N), C(N);
  for (int i = 0; i < N; i++) {
    R.at(i) = get<0>(RC.at(i));
    C.at(i) = get<1>(RC.at(i));
  }
  
  // 1点更新・区間最大値
  auto fx = [](pair<int, int> x1, pair<int, int> x2) -> pair<int, int> { return max(x1, x2); };
  const int MX = 2e5+1;
  SegTree<pair<int, int> > seg(MX, fx, pair<int, int>(0, -1));
  for (int i = 0; i < MX; i++) {
    seg.set(i, pair<int, int>(0, -1));
  }
  seg.build();  // 構築

  vector<int> pre(N, -1);
  for (int i = 0; i < N; i++) {
    auto [lis, p] = seg.query(0, C.at(i) + 1);
    pre.at(i) = p;
    seg.update(C.at(i), make_pair(lis + 1, i));
  }
  auto [lis, p] = seg.query(0, MX + 1);
  string ans;
  int r = H, c = W;
  while (p != -1) {
    while (r > R.at(p)) {
