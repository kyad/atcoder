...
int main() {
  ...
  for (int k = 0; k < K; k++) {
    ...
    P MINF = make_pair(-INF, -N - 2);
    SegTree<P> seg(N + 1, fx, MINF);
    for (int i = 0; i <= N; i++) {
      seg.set(i, make_pair(S.at(i), -i));
    }
    seg.build();
    long long mx = -INF;
    int optl = -1;
    int optr = -1;
    for (int l = 0; l <= N - 1; l++) {
      P opt = seg.query(l + 1, N + 1);
      long long now = opt.first - S.at(l);
      int nowr = -opt.second;
      if (now > mx) {
        optl = l + 1;
        optr = nowr;
