...
int main() {
  ...
  vector<long long> P(N + 1, 0), Q(N + 1, 0), R(N + 1, 0);
  for (int i = 1; i <= N; i++) {
    P.at(i) = P.at(i - 1) + A.at(i);
    Q.at(i) = Q.at(i - 1) + B.at(i);
    R.at(i) = R.at(i - 1) + C.at(i);
  }
  // 1点更新・区間最大値
  auto fx = [](long long x1, long long x2) -> long long { return max(x1, x2); };
  SegTree<long long> seg(N + 1, fx, -INF);
  // 初期化 i: 0-indexed
  for (int i = 1; i <= N; i++) {
    seg.set(i, Q.at(i) - R.at(i));
  }
  seg.build();  // 構築

  long long ans = -INF;
  for (int x = 1; x <= N - 2; x++) {
    long long mx = seg.query(x + 1, N);
    long long now = -P.at(0) + P.at(x) - Q.at(x) + mx + R.at(N);
    ans = max(ans, now);
  }
  cout << ans << '\n';
  return 0;
