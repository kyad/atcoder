...
int main() {
  ...
  long long D;
  cin >> N >> D;
  vector<pair<long long, long long>> AC(N);
  for (int i = 0; i < N; i++) {
  ...
  for (int i = 0; i < N; i++) {
    seg.set(i, AC.at(i).second);
  }
  seg.build();  // 構築

  long long ans = INF;
  for (int i = 0; i < N; i++) {
    long long A = AC.at(i).first;
    long long C = AC.at(i).second;
    if (A >= D) {
      ans = min(ans, C);
    } else {
      pair<long long, long long> p = make_pair(D - A, 0);
      auto it = lower_bound(AC.begin(), AC.end(), p);
      if (it != AC.end()) {
        int l = it - AC.begin();
        l = max(i + 1, l);
        if (l >= N) {
