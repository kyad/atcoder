...
int main() {
  ...
  cin >> N;
  int n = N.size();
  vector<vector<vector<long long> > > dp(n + 1, vector<vector<long long> >(1<<10, vector<long long>(2, 0)));
  vector<vector<vector<long long> > > ds(n + 1, vector<vector<long long> >(1<<10, vector<long long>(2, 0)));
  // j == 0: Nと一致
  // j == 1: N未満
  for (int i = 0; i < n; i++) {
    int ni = i + 1;
