...
template<class T1, class T2, class T3, class T4, class T5> ostream& operator<<(ostream& os, const tuple<T1, T2, T3, T4, T5> &t) { os << "(" << get<0>(t) << ", " << get<1>(t) << ", " << get<2>(t) << ", " << get<3>(t) << ", " << get<4>(t) << ")"; return os; }
...
template<class T> ostream& operator<<(ostream& os, const set<T> &s) {
  ...
  os << v; return os; }
template<class T> ostream& operator<<(ostream& os, const set<T, greater<T> > &s) {
  vector<T> v; for (typename set<T>::iterator it = s.begin(); it != s.end(); it++) { v.push_back(*it); };
  os << v; return os; }
template<class T> ostream& operator<<(ostream& os, const multiset<T> &ms) {
  ...
  os << v; return os; }
template<class T> ostream& operator<<(ostream& os, const multiset<T, greater<T> > &ms) {
  vector<T> v; for (typename multiset<T>::iterator it = ms.begin(); it != ms.end(); it++) { v.push_back(*it); };
  os << v; return os; }
...
int main() {
  ...
  T.at(2) = Y;
  for (int j = 3; j < M; j++) {
    T.at(j) = T.at(j - 1) + T.at(j - 2);
  }
  string S = "_" + T.at(M - 1);
  print(S);
  // 1~k文字目にcが含まれる個数
  auto f = [&](long long k, char c) -> long long {
    long long ans = count(S.begin(), S.begin() + k + 1, c);
    print(k, c, ans);
    return ans;
  };
